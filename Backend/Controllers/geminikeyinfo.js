...
export const geminikeyinfo = async (req, res, next) => {
    try {
        ...
        const prompt = `This is my resume data in text format. Resume Data: ${responseData}. Check the whole resume including projects, experience, skills,
         tech stacks, etc., to find out what kind of job types this resume is targeting. Send me an array in JavaScript where the array contains only the job types,
          sub job types,related job types ,every job types the resume is targeting. If the resume is targeting only one job type, send an array with that one job type.
